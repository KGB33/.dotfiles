...
You write test cases (pressure scenarios with fresh agents), watch them fail (baseline behavior), write the skill (documentation), watch tests pass (agents comply), and refactor (close loopholes).

**Core principle:** If you didn't watch an agent fail without the skill, you don't know if the skill teaches the right thing.

**REQUIRED BACKGROUND:** You MUST understand `test-driven-development`. That skill defines the RED-GREEN-REFACTOR cycle; this skill adapts it to documentation.

**Official guidance:** Use [Pi's skill documentation](https://github.com/badlogic/pi-mono/blob/main/packages/coding-agent/docs/skills.md) for Pi discovery, invocation, and validation behavior, and the [Agent Skills specification](https://agentskills.io/specification) for the portable skill format.
...
Pi discovers directories containing `SKILL.md` recursively in its configured skill locations. It registers each discovered skill as `/skill:name`; the agent can also load a matching skill on demand from its name and description.
...
**Frontmatter (YAML):**
- Two required fields: `name` and `description` (see the [Agent Skills specification](https://agentskills.io/specification))
- `name`: 1-64 lowercase letters, numbers, and hyphens; no leading, trailing, or consecutive hyphens; it must match the parent directory name
- `description`: 1-1024 characters describing what the skill does and when to use it, with specific discovery keywords
...
Your agent reads the description to decide which skills to load. State what the skill does and the concrete situations that should trigger it.

**CRITICAL: Summarize the capability, not its full workflow.**

A detailed workflow summary can cause agents to skip the skill body. Keep the "what" concise, then use the "when" for concrete triggers and discovery keywords.

```yaml
# ❌ BAD: Summarizes workflow - agents follow this instead of reading the skill
description: Use when validating APIs - checks schemas, runs requests, and publishes a report

# ❌ BAD: Too abstract, no trigger
description: For async testing

# ✅ GOOD: Concise capability plus concrete triggers
description: Replaces timing guesses with condition-based waits. Use when tests are flaky, timing-dependent, or intermittently fail.
```

**Description content:**
- A concise statement of what the skill enables
...
**Cross-referencing other skills:** use the unnamespaced skill name with an explicit requirement marker — `**REQUIRED SUB-SKILL:** Use test-driven-development`. Never use `@` file links: they force-load immediately and burn context.
...
**RED (baseline):** Run pressure scenarios with a fresh agent in an isolated context WITHOUT the skill. Document verbatim what it chose, what rationalizations it used, and which pressures triggered violations. You must see what agents naturally do before writing the skill.
...
1. **One fresh-context sample per call** — raw API call or isolated single-shot model invocation. System prompt = the realistic context the guidance will live in; user message = a task that tempts the failure.
...
**Testing methodology:** See the [fresh-agent testing methodology](testing-skills-with-subagents.md) for pressure scenario design, pressure types, and hole-plugging.
...
- [ ] Name is 1-64 lowercase letters, numbers, and hyphens; no leading, trailing, or consecutive hyphens; it matches the parent directory name
- [ ] YAML frontmatter has `name` and `description`; description is 1-1024 characters
- [ ] Description states what the skill does and when to use it, with concrete discovery keywords
